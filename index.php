...
<body>
  ...
  if (isset($_GET['cari'])) {
    $cari = $_GET['cari'];
    $sql = "select * from input where merk like '%$cari%' order by id desc";
  } else {
    $sql = "SELECT * FROM input ORDER BY id";
  }
  ...
  ?>
    <tr>
      <td><?php echo $id++ ?></td>
      <td><?php echo $input['merk'] ?></td>
      <td><?php echo $input['tipe'] ?></td>
      <td><?php echo $input['harga'] ?></td>
      <td><?php echo $input['jumlah'] ?></td>
      <td>
        <a href="edit.php?id=<?php echo $input['id']; ?>"><i class="fas fa-edit"></i></a> &nbsp;
        <a href="hapus.php?id=<?php echo $input['id']; ?>" onclick="return confirm('apakah anda yakin ingin menghapusnya?');"><i class="fas fa-trash"></i></a>
      </td>
    </tr>
  <?php } ?>
  </table>
